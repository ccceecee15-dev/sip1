...
const STORE_FORMATS = ["Flagship", "Express", "Outlet", "Premium", "Discount"];
...
export default function ClosingStockProducts() {
  // Filters that sync with drill-down
  ...
  // Generate product view data based on drill level and filters
  const getProductViewData = () => {
    if (drillLevel === "category") {
      ...
      const subs = PRODUCT_SUBCATEGORIES[selectedProductCategory] || [];
  ...
  const productViewData = getProductViewData();
  ...
  // Handle filter changes - update drill level and selection
  const handleCategoryChange = (value: string) => {
    setSelectedCategory(value);
    if (value === "all") {
      setDrillLevel("category");
      setSelectedProductCategory("Books");
    } else {
      setDrillLevel("subcategory");
  ...
  const handleMerchAreaChange = (value: string) => {
    setSelectedMerchArea(value);
    if (value === "all") {
      setDrillLevel("merchArea");
    } else {
      setDrillLevel("planningGroup");
  ...
  const handlePlanningGroupChange = (value: string) => {
    setSelectedPlanningGroup(value);
  };
    ...
    <div className="bg-card border rounded-lg p-4 shadow-sm space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div>
          ...
          <Select value={selectedMerchArea} onValueChange={handleMerchAreaChange}>
            <SelectTrigger className="h-8 text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Select value={selectedPlanningGroup} onValueChange={handlePlanningGroupChange}>
            <SelectTrigger className="h-8 text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
    ...
    return (
      <div className="h-screen w-screen bg-background overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h1 className="text-lg font-semibold">Closing Stock by Product</h1>
              ...
              <Table className="text-xs">
                <TableHeader className="sticky top-0 bg-card z-10 shadow-sm">
                  <TableRow className="bg-muted/30 border-b-2 border-b-muted h-12">
                    <TableHead className="sticky left-0 bg-background z-20 border-r w-[100px] font-bold text-center">{drillLevelLabels[drillLevel]}</TableHead>
                ...
                <TableBody>
                  {productViewData.map((row: any, idx) => (
                    <TableRow key={idx} className="hover:bg-muted/30 cursor-pointer">
                      <TableCell className="sticky left-0 bg-background border-r font-medium z-10 text-center text-xs">
                        {drillLevel !== "planningGroup" ? (
                          <div className="text-primary hover:underline" onClick={() => handleProductDrill(row.label)}>{row.label} →</div>
                        ) : (
                          row.label
                        )}
                      </TableCell>
            ...
            {drillLevel !== "category" && (
              <div className="px-4 py-3 border-b bg-muted/20 flex items-center gap-2">
                <Button 
                  ...
                  onClick={handleProductDrillBack}
                >
                  ← Back
                </Button>
                <span className="text-xs text-muted-foreground">
                  ...
                  {selectedProductCategory && (
                    <span className="ml-2 font-medium text-foreground">&gt; {selectedProductCategory}</span>
                  )}
                </span>
              </div>
            )}
                  ...
                  <TableBody>
                    {productViewData.map((row: any, idx) => (
                      <TableRow 
                        ...
                        onClick={() => drillLevel !== "planningGroup" && handleProductDrill(row.label)}
                      >
                        <TableCell className="sticky left-0 bg-background border-r font-medium z-10 text-center text-xs">
                          {drillLevel !== "planningGroup" ? (
