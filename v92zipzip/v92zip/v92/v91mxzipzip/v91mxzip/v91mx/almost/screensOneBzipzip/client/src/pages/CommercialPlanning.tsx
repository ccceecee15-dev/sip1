...
import { Download, Calendar, Grid3x3, Grid2x2, Clock, ArrowUpDown, ArrowUp, ArrowDown, BarChart3 } from "lucide-react";
...
export default function CommercialPlanning() {
  ...
  const handleApplyChanges = () => {
    ...
    setEditedCells({});
    toast.success("Commercial planning changes applied successfully.");
  };
      ...
      <MainLayout>
        <div className="flex items-center justify-center h-96">
          <div className="flex items-center gap-3 text-muted-foreground">
            ...
            <span>Loading Commercial Planning Data...</span>
          </div>
        </div>
      </MainLayout>
    );
  ...
  return (
    <MainLayout>
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <CompactFilterBar
          onApply={() => setFiltersApplied(true)}
          onClear={() => {
        ...
        <Card className="relative overflow-hidden rounded-xl glass-card border shadow-lg shadow-slate-200/40 dark:shadow-slate-950/40">
          <CardHeader className="py-3 px-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            <div className="flex flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3.5">
                <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-violet-600/10 dark:bg-violet-500/10 text-violet-600 dark:text-violet-400">
          ...
          <CardContent className="p-0">
            {!filtersApplied ? (
              <div className="flex flex-col items-center justify-center py-24 px-6 text-center bg-slate-50/50 dark:bg-slate-900/50">
                <div className="w-16 h-16 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center mb-4">
                  <Grid3x3 className="h-8 w-8 text-violet-600 dark:text-violet-400" />
                </div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">Ready to Plan?</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm mx-auto mb-6">
                  Select your Category, Sub category, and Merch Area from the filters above, then click Apply Filters to view your commercial plan.
                </p>
                <div className="flex items-center gap-2 text-xs font-medium text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-900/20 px-3 py-1.5 rounded-full">
                ...
                <table className="w-full text-[11px]">
                  <thead className="sticky top-0 z-10">
                    <tr className="border-b-2 border-slate-200 dark:border-slate-700">
                      <th colSpan={2} className="bg-slate-50 dark:bg-slate-900 px-2 py-1.5 text-[9px] font-bold uppercase tracking-wider border-r border-slate-200 dark:border-slate-700"></th>
                      <th colSpan={10} className="bg-blue-50/60 dark:bg-blue-950/30 px-2 py-1.5 text-[9px] font-bold uppercase tracking-wider text-blue-700 dark:text-blue-300 border-r border-slate-200 dark:border-slate-700 text-center">Sales Retail</th>
                      <th colSpan={2} className="bg-emerald-50/60 dark:bg-emerald-950/30 px-2 py-1.5 text-[9px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300 border-r border-slate-200 dark:border-slate-700 text-center">CoGS</th>
                    </tr>
                    <tr>
                      ...
                      <TableHeader className="bg-slate-50 dark:bg-slate-900 min-w-[100px] border-r border-slate-200 dark:border-slate-700">CP Plan Applied</TableHeader>
                      <TableHeader group="sales" className="min-w-[100px]">LY Actuals</TableHeader>
                      <TableHeader group="sales" className="min-w-[100px]">Budget</TableHeader>
                      <TableHeader group="sales" className="min-w-[80px]">FC1</TableHeader>
                      <TableHeader group="sales" className="min-w-[80px]">FC2</TableHeader>
                      <TableHeader group="sales" className="min-w-[100px]">CP Plan</TableHeader>
                      <TableHeader group="sales" className="min-w-[100px]">TY Actuals</TableHeader>
                      <TableHeader group="sales" className="min-w-[90px]">vs Budget</TableHeader>
                      <TableHeader group="sales" className="min-w-[90px]">vs FC1</TableHeader>
                      <TableHeader group="sales" className="min-w-[90px]">vs FC2</TableHeader>
                      <TableHeader group="sales" className="min-w-[90px]">vs CP</TableHeader>
                      <TableHeader group="stock" className="min-w-[120px]">Avg Trading Margin</TableHeader>
                      <TableHeader group="stock" className="min-w-[120px]">CP Plan at Cost</TableHeader>
                    </tr>
                  </thead>
                    ...
                    {sortedData.map((row, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-2 py-1.5 text-center border-r border-slate-200 dark:border-slate-700">
                             <Badge variant="outline" className="text-[9px] font-normal uppercase py-0 px-1 bg-slate-50">
                               {row.period < 5 ? "-" : row.period === 5 ? "No" : "Yes"}
                             </Badge>
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={row.period === 1 ? 474035 : row.period === 2 ? 511162 : row.period === 3 ? 512999 : row.period === 4 ? 613080 : row.period === 5 ? 519690 : row.period === 6 ? 563213 : row.period === 7 ? 551835 : row.period === 8 ? 551913 : row.period === 9 ? 557810 : row.period === 10 ? 585105 : row.period === 11 ? 600578 : row.period === 12 ? 642282 : 606555} type="currency" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={row.period === 1 ? 650372 : row.period === 2 ? 839931 : row.period === 3 ? 652101 : row.period === 4 ? 640322 : row.period === 5 ? 689515 : row.period === 6 ? 617141 : row.period === 7 ? 674236 : row.period === 8 ? 878902 : row.period === 9 ? 738435 : row.period === 10 ? 785493 : row.period === 11 ? 990957 : row.period === 12 ? 771084 : null} type="currency" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={0} type="currency" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={0} type="currency" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <div className="flex items-center justify-end">
                              <input
                                type="number"
                                className={cn(
                                  "w-20 h-6 px-1 text-right border rounded focus:outline-none focus:ring-1 focus:ring-violet-500",
                                  row.period < 5 ? "bg-slate-100 dark:bg-slate-800 text-slate-500" : "bg-white dark:bg-slate-950"
                                )}
                                readOnly={row.period < 5}
                                value={editedCells[tgtKey] ?? (
                                  row.period === 1 ? 650372 :
                                  row.period === 2 ? 839931 :
                                  row.period === 3 ? 652101 :
                                  row.period === 4 ? 640322 :
                                  ...
                                  row.period === 6 ? 656565 :
                                  row.period === 7 ? 656565 :
                                  row.period >= 8 && row.period <= 11 ? (row.period === 10 ? 7656777 : 7656756) :
                                  row.period === 12 ? 765678 :
                                  row.period === 13 ? 585866 : 0
                                )}
                                onChange={(e) => handleEditCell(tgtKey, parseFloat(e.target.value) || 0)}
                              />
                              {editedCells[tgtKey] !== undefined && <div className="w-1 h-1 rounded-full bg-violet-500 ml-1" />}
                            </div>
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={row.period === 1 ? 600516 : row.period === 2 ? 643220 : row.period === 3 ? 605980 : row.period === 4 ? 678484 : row.period === 5 ? 372174 : null} type="currency" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={row.period === 1 ? -7.67 : row.period === 2 ? -23.42 : row.period === 3 ? -7.07 : row.period === 4 ? 5.96 : row.period === 5 ? -46.02 : null} type="percentChange" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={null} type="percentChange" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10">
                            <MetricCell value={null} type="percentChange" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-blue-50/20 dark:bg-blue-950/10 border-r border-slate-200 dark:border-slate-700">
                            <MetricCell value={row.period === 5 ? -57.55 : null} type="percentChange" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-emerald-50/20 dark:bg-emerald-950/10">
                            <MetricCell value={row.period === 1 ? 60.47 : row.period === 2 ? 60.22 : row.period === 3 ? 59.59 : row.period === 4 ? 62.40 : 62.40} type="percent" />
                          </td>
                          <td className="px-2 py-1.5 text-right bg-emerald-50/20 dark:bg-emerald-950/10">
                            <MetricCell value={row.period === 5 ? 329665 : row.period === 6 ? 246868 : row.period === 7 ? 246868 : row.period === 8 ? 2878940 : row.period === 9 ? 2878940 : row.period === 10 ? 2878948 : row.period === 11 ? 2879279 : row.period === 12 ? 287895 : row.period === 13 ? 220286 : null} type="currency" />
                          </td>
                        </motion.tr>
                      );
                    })}
