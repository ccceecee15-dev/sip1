...
import {
  ...
  ChevronsUpDown, Layers, ShoppingCart, AlertCircle,
} from "lucide-react";
...
const STORE_NAMES_LIST = [
  "TECH CENTRAL-DOWNTOWN", "GADGET ZONE-LAKESIDE", "DIGITAL HUB-CANARY WHARF",
  "SMART STORE-BLUEWATER", "MOBILE WORLD-WESTFIELD", "CONNECT-HEATHROW T5",
  "TECH WORLD-BULLRING", "MOBILE PLUS-TRAFFORD CTR", "CONNECT-GATWICK SOUTH",
  "TECH HUB-GLASGOW BUCHANAN", "GADGET PRO-BRAEHEAD", "CONNECT-STANSTED",
...
const SAVED_VIEW_CONFIGS: Record<string, ViewConfig> = {
  "Deficit View":       { visibleGroups: ["product", "sipMetrics"], hiddenCols: ["reorderableFlag","primaryCurrentCost"] },
...
function generateSIPData(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const suggestedOrderQty = deficit > 0
      ? Math.ceil(deficit / orderMult) * orderMult
      : 0;
...
function deficitLevel(v: number): "high" | "medium" | "none" {
...
export default function SIPPlanning() {
  ...
  const [filters, setFilters] = useState({ brand: "all", plcStatus: "all", search: "" });
  ...
  const [selectedView, setSelectedView]         = useState("Deficit View");

  // Default sort: highest deficit first
  const [sortCol, setSortCol] = useState<string | null>("deficit");
  ...
  const summary = useMemo(() => {
    const withDeficit = filteredData.filter((r) => r.deficit > 0);
    return {
      totalStyles:   filteredData.length,
      deficitStyles: withDeficit.length,
      totalDeficit:  withDeficit.reduce((s, r) => s + r.deficit, 0),
      totalOrderQty: filteredData.reduce((s, r) => s + r.suggestedOrderQty, 0),
    };
  }, [filteredData]);
  ...
  const handleReset = () => {
    ...
    setSortCol("deficit"); setSortDir("desc");
    setPage(1); setRowsPerPage(25); setSelectedView("Deficit View");
  };
  const handleExport = () => {
  ...
  const renderCell = (col: ColDef, val: any) => {
    ...
    if (col.id === "deficit") {
      const level = deficitLevel(Number(val));
      return (
        <span className={cn("font-bold tabular-nums", level === "high" ? "text-red-600" : level === "medium" ? "text-orange-500" : "text-slate-500")}>
      ...
      <div className="space-y-4 animate-in fade-in duration-500">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              {filteredData.length} styles · <span className="text-red-600 font-semibold">{summary.deficitStyles} with deficit</span> · {summary.totalOrderQty.toLocaleString()} units to order
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <Button
              size="sm" className="gap-1.5 text-xs font-bold bg-red-600 hover:bg-red-700 text-white"
              onClick={() => navigate("/reports/sip-planning/deficit-by-location")}
            >
              <Layers className="h-3.5 w-3.5" /> View Deficit by Location
            </Button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "Styles with Deficit", value: summary.deficitStyles.toLocaleString(), color: "text-red-600",    bg: "bg-red-50",         icon: AlertCircle },
            { label: "Total Deficit Units", value: summary.totalDeficit.toLocaleString(),  color: "text-orange-600", bg: "bg-orange-50",      icon: AlertCircle },
            { label: "Units to Order",     value: summary.totalOrderQty.toLocaleString(),  color: "text-emerald-600", bg: "bg-emerald-50",    icon: ShoppingCart },
            ...
            {filteredData.length === 0 ? (
              ...
              <div className="overflow-auto max-h-[calc(100vh-520px)]">
                <table className="border-collapse text-[11px] w-max min-w-full">
                  ...
                  <tbody>
                    {pageData.map((row, ri) => {
                      const defLevel  = deficitLevel(row.deficit);
                      let frozenTrack = 0;
                      return (
                        <tr
                          key={`row-${ri}`}
                              ...
                              return (
                                <td key={col.id}
                                  className={cn(
                                    ...
                                    col.id === "deficit" && defLevel === "high" && "bg-red-100/70",
                                    col.id === "deficit" && defLevel === "medium" && "bg-orange-50/70",
                                  )}
                                >
                                  {renderCell(col, val)}
                                </td>
